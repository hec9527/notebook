...
def task_getGoodsInfo(taskIndex, lis, flag):
    '''
        子线程任务\n
        taskIndex-->线程的索引\n
        lis-->当前的商品的URL以及价格信息\n
    ...
    try:
        ...
        msg.insert(ms, 0)
        msg.pop(1)
        task_getGoodsInfo(taskIndex, lis, flag)
        return
    ...
    task_saveGoodsInfo(taskIndex, strs, flag)
...
def taskMain(Tindex, Pstart, Pend):
    ...
    for i in range(Pstart, Pend):
        ...
        for u in lis:  # 遍历列表中的所有数据
            if i >= Pend - 1 and u == lis[-1]:  # 是否是该任务的最后一个页面  并且是该列表中的最后一个商品
                task_getGoodsInfo(Tindex, u, True)
            else:
                task_getGoodsInfo(Tindex, u, False)
        percent = (i - Pstart) / (Pend - Pstart)  # 当前任务的进度
