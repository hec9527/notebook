...
class Game():
    def __init__(self):
        ...
        self.size = 1600, 880
        self.surface_bg = pygame.display.set_mode(self.size)  # surface 背景层  , FULLSCREEN | DOUBLEBUF
    ...
    def setTips(self):
        ''' 设置游戏中的提示 '''
        self.surface_bg.blit(self.font_title.render(self.tips[0], True, self.color_rect), (1035, 130))
        t=150
        for i in range(1, len(self.tips)):
            t += 30
            self.surface_bg.blit(self.font_text.render(self.tips[i], True, self.color_rect), (1070, t))
        self.surface_bg.blit(self.font_title.render(self.operates[0], True, self.color_rect), (1035, 530))
        t = 550
        for i in range(1, len(self.operates)):
            t+=30
            self.surface_bg.blit(self.font_text.render(self.operates[i], True, self.color_rect), (1070, t))
        self.surface_bg.blit(self.font_head.render("当前分数", True, self.color_black), (765, 150))
        t = 425
        for i in range(1,len(self.btnMsg)):
            self.surface_bg.blit(self.font_btn.render(self.btnMsg[i], True, self.color_rect), (820, t))
            t += 65
        self.setBtnReturn()
    ...
    def setBtnReturn(self):
        self.surface_bg.blit(self.font_btn.render(self.btnMsg[0] + self.getReturn() + "次", True, self.color_rect), (820, 360))
