...
while 1:
    ...
    imgShow = pygame.transform.smoothscale(imgShow, (int(imgRect.width * ratio), int(imgRect.height * ratio)))
    # smoothscale 函数   二个参数 第一个surface 第二个缩放后的宽高

    imgRect.left, imgRect.top = pos[0], pos[1]  # 设置矩形方块的位置
    screen.fill((255, 255, 255))
    screen.blit(imgShow, imgRect)   # 两个参数  第一个  需要绘制的图像  surface  第二个 绘制到的矩形方块  pygamge.rect
                                    # 将surface 绘制到 rect
