...
class Ball(pygame.sprite.Sprite):
    def __init__(self, imgs, postion, speed, w, h):
        ...
        self.image = pygame.image.load(self.imList[self.imIndex]).convert_alpha()
    ...
    def move(self):
        ...
        if self.rect.left > self.fatherShape[0] - self.rect.width:
            self.speed[0] = -self.speed[0] if self.speed[0] > 0 else self.speed[0]
        if self.rect.left < 0:
            self.speed[0] = -self.speed[0] if self.speed[0] < 0 else self.speed[0]
    ...
    while run:
        ...
        for i in range(len(ballLst)):
            ...
            if collid_ball(item, ballLst):
                ...
                item.image = pygame.image.load(item.imList[item.imIndex]).convert_alpha()
