'''
 * @Author: hechun 
 * @Date: 2018-10-15 12:39:06 
 * @Last Modified by:   hechun 
 * @Last Modified time: 2018-10-17 16:39:06 
'''
import turtle,time

# 优化方向：
# 1.汉字不会修改可以使用 一个函数单独绘制不用反复擦除刷新 
# 2.减少对象创建合并部分画笔工具 例如 年月日星期的画笔工具 使用频率很低 可以合并减少 内存占用
# 3.合并画笔工具  它们不同的各自的属性可以使用序列存放  单独调用  相同的属性不用需改 
# @ 2018-10-17 16:35 by hc
...
def init():
    ...
    for i in range(0,len(text)):    # 利用循环生成  turtle对象  设置属性并且放置在列表里面
        ...
        if 3>i>=0:  # 设置第 1-3 个画笔的属性
            ...
            printer[i].numSpace=10
            printer[i].fontSize=24
            printer[i].cpos=[-250+(170*(i-3)),50]   
