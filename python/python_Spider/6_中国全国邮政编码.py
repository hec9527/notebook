...
def getCityPost(_url):
    ...
    # 获取并且剪切需要匹配的文本
    string = urllib.request.urlopen(_url, timeout=12).read().decode(
        "GBK")  # 剪切页面中有关省下级的市的信息
    ...
    for i in range(len(result_city)):
        ...
        if result_cityPost[i][-6:][2] == "/":
            writePost(result_city[i][3:-8] + "\t" +
                      result_cityPost[i][-6:][3:], 1)  # 写入 市 的信息
        else:
            writePost(result_city[i][3:-8] + "\t" + result_cityPost[i][-6:],
                      1)  # 写入 市 的信息
        # 截取页面中相应的市下面的县城 信息传递到 getCountyPost() 获取每个县城的邮编
        if i == len(result_cityPost) - 1:
            _string = string[string.find(result_cityPost[-1]):-1]
        else:
            _string = string[string.find(result_cityPost[i]):string.
                             find(result_cityPost[i + 1])]
...
def getCountyPost(string):
    ...
    for i in range(len(result_county)):
        if result_countyPost[i][-6:][2] == "/":
            writePost(
                result_county[i][4:result_county[i].find("</td>")] + "\t" +
                result_countyPost[i][-6:][3:], 2)
        else:
            writePost(
                result_county[i][4:result_county[i].find("</td>")] + "\t" +
                result_countyPost[i][-6:], 2)
